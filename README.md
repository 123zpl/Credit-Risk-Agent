...
> 面向演示与研究场景的演进路线；优先级可按课题排期调整。

### 近期（Agent 与风控核心）

- [ ] **优化 Agent 流程**：收紧 Supervisor `execution_plan`（减少冗余 Worker 调用）；完善 `entry_router` / 多轮指代；Worker 输出结构化 schema，便于 `supervisor_respond` 汇总  
- [ ] **审批决策护栏**：规则引擎优先于 LLM 结论；合规节点一票否决；政策 RAG 引用与决策字段校验  
- [ ] **可观测性**：LangGraph / Celery 节点耗时与失败原因落库；前端展示审批 DAG 当前步骤（替代纯轮询文案）

### 中期（体验与工程化）

- [ ] **分析对话增强**：流式输出（SSE）或分步展示 Worker 中间结果；会话导出与报告归档  
- [ ] **评估与回归**：固定评测集（查数 / 归因 / 审批样本）；对比 plan 路由与审批通过率基线  
- [ ] **安全与运维**：`.env.example`、密钥轮换说明；API 限流与审计日志；Milvus / MySQL 健康检查纳入 `/health`  
- [ ] **前端补图与交互**：`/apply`、风险归因等界面截图；批量审批进度条；报告 PDF 导出

### 探索（可选）

- [ ] **人机协同**：低置信度申请人转人工复核队列  
- [ ] **记忆升级**：长期记忆向量检索 + 摘要压缩策略 A/B  
- [ ] **多模型路由**：简单查数用小模型，复杂规划/审批报告用大模型，降低成本  
